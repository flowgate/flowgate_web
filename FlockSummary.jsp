<%@ page import="org.immport.struts.utils.ImmportConfig" %>
<%@ taglib uri="/WEB-INF/immport-help.tld" prefix="help" %>
<%@ taglib uri="/WEB-INF/c.tld" prefix="c"%>
<%@ taglib uri="/WEB-INF/struts-html.tld" prefix="html"%>

<script type="text/javascript" src="<%= request.getContextPath() %>/files/scripts/prototype.js"></script>
<link rel="stylesheet" type="text/css" href="<%= request.getContextPath() %>/files/scripts/ext-2.2/resources/css/ext-all.css" />
<link rel="stylesheet" type="text/css" href="<%= request.getContextPath() %>/files/stylesheets/immport-xtheme-gray.css" />
<script type="text/javascript" src="<%= request.getContextPath() %>/files/scripts/ext-2.2/adapter/ext/ext-base.js"></script>
<script type="text/javascript" src="<%= request.getContextPath() %>/files/scripts/ext-2.2/ext-all.js"></script>	
<link rel="stylesheet" type="text/css" href="<%= request.getContextPath() %>/files/scripts/ext-2.2/resources/css/columnLock.css" />
<script type="text/javascript" src="<%= request.getContextPath() %>/files/scripts/ext-2.2/columnLock.js"></script>
...
<div id="flock_main" style="height:600px;vertical-align=top;">
<div style="background-color:#ffffff;color:#000000;" id="flock_task_name"></div>
<div id="flock_tool_bar"></div>
<div id="flock_content" style="color:#000000;"></div>
</div>
    ...
    conn.request( {
        ...
        success: function(resp,opt) {
            ...
            var imagePanel = new Ext.Panel( {
                ...
                bodyStyle: imageURL,
                divs: [ {x:editConfig.data.xCoord-5, y:editConfig.data.yCoord-5} ],
...
var loadSplitCentroid = function(idx1,idx2,popId) {
    ...
    conn.request( {
        ...
        success: function(resp,opt) {
            ...
            var x1 = parseInt(xCoord) - 5;
            var y1 = parseInt(yCoord) - 5;
            var x2 = parseInt(xCoord) + 5;
            var y2 = parseInt(yCoord) - 5;
...
Ext.onReady(function() {
    Ext.QuickTips.init();
    Ext.get('flock_task_name').dom.innerHTML = '<h2 style="font-size: 1.8em;">Result Adjustment System(RAS) - Task Name: ' + taskName
     + '&nbsp;&nbsp;<a href="/help/Flock_Analysis_Help.pdf"><img src="<%= request.getContextPath() %>/images/how_to.gif" width="16" height="16" border="0" alt=""></a></h2>';
	loadToolBar();
	loadOverview("color");
