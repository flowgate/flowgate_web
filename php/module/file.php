...
class FileModule {
    ...
    function addFile($_name, $_desc, $_pid, $_uid) {
    	$this->dbm();
        $con = $this->dbModule->connect();
        $this::$SUCCESS = $this->dbModule->addFile($con, $_name, $_desc, $_pid, $_uid);
    } 
	...
	function getFiles($_uid, $_pid, $_fid) {
        ...
        $json = null;
        if(!is_null($result)) {
            $fields = array("f_id","f_name","f_desc","f_org_name","f_status","p_name");
            $rows = array();
