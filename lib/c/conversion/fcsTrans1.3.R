...
processfcsfile <- function(fcsfile, verbose=F, overwrite=F,
    renameSource=F, convert=T, savekeys=F) {
    ...
    #pieces[length(pieces)] = 'Transformation_Results'
	#folder_out = paste(pieces, collapse = .Platform$file.sep)
	
	#if (!file.exists(folder_out)) {
    #    dir.create(folder_out)
    #    Sys.chmod(folder_out, mode="0777")
    #}
	#pieces[length(pieces)+1]= paste(filepieces,collapse = '.')
    ...
    if (doconvert) {
        ...
        if (convert) {
            ...
            #write.table(cdata, file=file.path(out_dir,channelfile), quote=F, row.names=F, col.names=T, sep='\t', append=F)
			write.table(cdata, file=channelfile, quote=F, row.names=F, col.names=T, sep='\t', append=F)
        ...
        if (file.exists(entry)) {
            if (file.info(entry)$isdir) {
                ...
                if (length(files) > 0) {
                    ...
                    sapply(files, ipconvert, level=level+1, verbose=verbose, overwrite=overwrite, renameSource=renameSource)
                }
            } else {
                pieces = unlist(strsplit(entry, .Platform$file.sep))
                filename = pieces[length(pieces)]
                filepieces = unlist(strsplit(filename, '\\.'))

                # output_dir = paste('.', 'Transformation_Results', sep=.Platform$file.sep)
                ...
                if(filepieces[2] == "zip") {
                    ...
                    if (length(fcs_files) > 0) {
                        ...
                        #    Sys.chmod(output_dir, mode="0777")
                        #}
                        sapply(paste('.', fcs_files, sep=.Platform$file.sep), ipconvert, level=level+1, verbose=verbose, overwrite=overwrite, renameSource=renameSource)
                        
                        zip(paste('.', 'output.zip', sep=.Platform$file.sep), list.files('.', pattern = "\\.txt$"))
                    }
                } 
                #else if(filepieces[2] == 'txt') {
                #    file.rename(entry, paste(output_dir, filename, sep=.Platform$file.sep))
                #} 
                else {
                    isvalid = F
                    tryCatch({
                    ...
                    if (isvalid) {
                        ...
                        processfcsfile(entry, verbose=verbose, overwrite=overwrite,renameSource=renameSource)
                    } else {
...
# New entry point that GenePattern will call
run_ipconvert <- function(...) {
  options("warn"=-1)
  suppressPackageStartupMessages(suppressMessages(.run_ipconvert(...)))
}

.run_ipconvert <- function(entry, ...) {
    args <- c(...)
  ...
  for(i in 1:length(args))
  {
    ...
    arg <- substring(args[[i]], 5, nchar(args[[i]]))
    if(flag=='-li=') {
      libdir <- arg
    } else if(flag=='-ud=') {
      server.dir <- arg
  ...
  if((libdir == '') || (server.dir == '') ||  (patch.dir == '')) {
    stop("Could not figure out some of the directories (libdir, server.dir, or patch.dir)... check the source code!", call.=FALSE)
  }
