        ...
        if (file.exists(entry)) {
            if (file.info(entry)$isdir) {
                ...
                if(filepieces[2] == "zip") {
                    ...
                    files = list.files('.')
                    if (length(files) > 0) {
                        files = paste('.', files, sep=.Platform$file.sep)
                        sapply(files, ipconvert, level=level+1, verbose=verbose, overwrite=overwrite, renameSource=renameSource)
                        
                        zip('output.zip', paste(output_dir, list.files(output_dir), sep=.Platform$file.sep))
                    }
                } else if(filepieces[2] == 'txt') {
                    print (entry)
                    print (paste(output_dir, filename, sep=.Platform$file.sep))
