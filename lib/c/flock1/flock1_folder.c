...
void getfileinfo(FILE *f_src, int *file_Len, int *num_dm, char *name_string)
{
  char src[LINE_LEN];
      ...
      if ((src[i]=='\t') && (prv!='\t')) //a complete word
        {
           ...
           * We no inter want to automatically remove Time column.
...
void readsource(FILE *f_src, int file_Len, int num_dm, double **uncomp_data)
{
...
/*
 * Filter the data so that only the data beinting to dense grids is left
 *
 * Output:
  ...
  for (i=0;i<file_Len;i++)
    {
      ...
      if (dense_grid_ID!=-1) //for point (i) beinting to dense grids 
        {			
...
int main (int argc, char **argv)
{
  ...
  char folder_name[LINE_LEN];
  char folder_name_temp[LINE_LEN];
 ...
 // int temp=0;
  ...
  temp_t=j;
  while ((folder_name[temp_t]!='/') && (folder_name[temp_t]!='\\') && (temp_t>=0))
  {
		  temp_t--;
		  if (temp_t<0)
			  break;
  ...
  temp_tt=0;
  folder_name_temp[0]='\0';

  for (t=temp_t+1;t<=j;t++)
  {
	  folder_name_temp[temp_tt]=folder_name[t];
	  temp_tt++;
  }

  strcpy(folder_name,folder_name_temp);
  ...
  getfileinfo(f_src, &file_Len, &num_dm, para_name_string); //get the filelength, number of dimensions, and num/name of parameters
  ...
  readsource(f_src, file_Len, num_dm, input_data); //read the data;
  ...
  //printf("before grid_To_event\n");
  grid_To_event(file_Len, dense_grid_reverse, all_grid_ID, eventID_To_denseventID, densegridID_To_gridevent);
  //printf("after grid_To_event\n");
  ...
  //printf("before generate_grid_seq\n");
  ...
  //printf("after generate_grid_seq\n");	
  /************************************************* allocate memory *********************************************/
  ...
  //			 int *cluster_ID, int *grid_ID, int *grid_clusterID)
  //printf("before merge_grids\n");	

  num_clust = merge_grids(normalized_data, interval, file_Len, num_dm, num_bin, position, num_dense_grids, dense_grid_reverse, dense_grid_seq, 
	  eventID_To_denseventID, densegridID_To_gridevent, all_grid_ID, cluster_ID, grid_ID, grid_clusterID);
